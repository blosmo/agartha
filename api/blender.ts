...
export default async function handler(req: BillingRequest, res: ServerResponse) {
  ...
  if ((!path || path === 'capabilities') && req.method === 'GET') {
    ...
    jsonResponse(res, {
      ...
      managed: { enabled: managedEnabled(discoveryToken, req), model: 'openai/gpt-6-astra', minimumBudgetCents: 100, maximumBudgetCents: 2000, references: { enabled: referencesEnabled(discoveryToken), model: 'openai/gpt-image-2.5-flare', minimumBudgetCents: 500 }, materials:{library:'/api/materials/library',guide:'/agents/material-authoring.md',publicationOption:'shareMaterials',publicationDefault:false}, jobs: '/api/blender/jobs' },
