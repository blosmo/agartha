...
def run_studio(broker: Any, files: ManagedFiles, token: str, job_id: str, executor: str,
    ...
    try:
        ...
        for turn in range(MAX_ACTIONS):
            ...
            if action not in {'inspect_scene', 'inspect_object', 'edit', 'render_views', 'accept', 'restore', 'finish', 'search_templates', 'inspect_template', 'build_template', 'search_assets', 'load_asset', 'search_polyhaven', 'load_polyhaven', 'prepare_asset', 'publish_asset', 'search_materials', 'load_material', 'prepare_material', 'publish_material'}: raise ValueError('Unknown Blender action.')
            ...
            try:
                ...
                if action == 'search_polyhaven':
                    ...
                    event['result'] = json.dumps(result, ensure_ascii=False)
                    trace()
                    history = f'Candidate revision {revision}. Current candidate accepted: {accepted_current}.\n' + json.dumps([event], ensure_ascii=False)
                    continue
