...
Known task IDs are polled by the existing broker recovery cycle after job cancellation or expiry. Recovery may settle charges or refunds but cannot create tasks. Every successful provider stage is retained before Blender import and exposed through an authenticated stage-labeled `generated-image-to-3d-*.glb` or `generated-rigging-*.glb` link. When the original task succeeds after its worker has stopped, recovery refreshes the task result, validates the current provider URL and creates the same durable link. This preserves the paid component as a partial artifact without reopening Blender or claiming final review. Canceling the job stops further work; it cannot undo a provider task already running. Incompatible usage or untrusted result URLs keep the hold unresolved for operator investigation rather than inventing a charge.

Generated GLBs and their reference/provenance are stored privately with the managed job before import. Final deliverables use the existing job artifact API and retention policy. Provider URLs can expire, so download successful assets promptly. Rigging is limited to suitable humanoid bipeds. When Meshy returns its basic walking GLB, that animated asset is retained and imported; if optional rigging fails after a valid textured generation, the worker continues with the saved unrigged base and records the fallback. Custom motion generation is outside this integration. Normal scene edits and final exported-model inspection still determine acceptance.

## Local verification

- Production build passed, including script and Convex type checks.
- All npm test stages passed: 912 tests across protocol, CLI, web, scripts, billing, playground, Convex, renderer and release checks. Local-server suites required localhost access outside the sandbox.
- All 220 Blender billing Python tests and 20 Blender MCP client tests passed. Retry tests model the broker's cached error responses and confirm one paid task with distinct Blender retry attempts.
