...
Poly Haven reuse is part of managed modeling by default, with no feature-specific environment variable. Deploy it as a coordinated worker, broker and web/gateway change. Rebuild the Blender worker image so it includes the updated `cloud/blender_mcp/components.py`; drain old workers and update the configured `AGARTHA_PAID_BLENDER_IMAGE_ID` before deploying the updated broker and action prompt. Retain unrelated current production changes.
