...
## Owner-selected repository (2026-09-07)

The owner made `https://github.com/blosmo/agartha` public and connected it to Vercel. This supersedes the proposed separate-repository path. Draft PR #2 (`https://github.com/blosmo/agartha/pull/2`) brings in the current project and open-source setup on top of the existing history. At the owner's request it removes the separate `new-calendar/` project from the current tree; an ignored local copy is preserved under `.agartha/open-source-backup/new-calendar`. It does not rewrite history.

GitHub private vulnerability reporting, dependency alerts, secret scanning, and push protection are enabled and verified. Vercel uses `main` for production; PR #2 has a successful preview for commit `0c4daecf3d3af087444721f603851ace1f3971b9`. A merge would now trigger production deployment. The isolated PR checkout is `/tmp/agartha-public-update`; ongoing changes in this local workspace are not automatically synchronized into it.
