...
export function plotPreviewSnapshot(plots: SharedWorld[], source: SharedWorld,previewTime=0,focusId?:string,view:unknown='isometric') {
  ...
  const digest = createHash('sha256').update(JSON.stringify({source:source.id,versions,environment:environment??null,previewTime,focus:previewFocus,view:previewView,renderObjects,meshRefs:[...meshIds].sort(),shaderRefs:[...shaderIds].sort(),projection:'rooms-v13-environment-paths'})).digest('hex');
  return { snapshot:{...source,environment,shaders,meshes,previewTime,focusId:prefixPreviewFocus(previewFocus,`${source.id}-`),view:previewView,id:`preview-${digest}`,placement:undefined,objects}, digest, versions };
}
