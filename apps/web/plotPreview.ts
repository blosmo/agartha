...
import {parsePreviewSize,type PreviewSize} from '../../packages/protocol/src/previewSize.js';
...
import {parseObjectMotion} from '../../packages/protocol/src/objectMotion.js';
import {parseRoomEnvironment} from '../../packages/protocol/src/roomEnvironment.js';
import type { SharedWorld, WorldObject } from './src/worlds/world.js';

export function plotPreviewSnapshot(plots: SharedWorld[], source: SharedWorld,previewTime=0,focusId?:string,view:unknown='isometric',size?:PreviewSize) {
  const previewView=parsePreviewView(view);
  const previewSize=size??parsePreviewSize(undefined,undefined);
  ...
  const renderObjects=objects.map(object=>({id:object.id,shape:object.shape,modelId:object.modelId??null,animation:object.animation?{clip:object.animation.clip,speed:object.animation.speed,paused:object.animation.paused}:null,meshId:object.meshId??null,position:object.position,scale:object.scale,color:object.color,yaw:object.yaw??0,motion:parseObjectMotion(object.motion)??null,materialId:object.materialId??null,shaderId:object.shaderId??null}));
  const environment=source.archived?undefined:parseRoomEnvironment(source.environment);
  const digest = createHash('sha256').update(JSON.stringify({source:source.id,versions,environment:environment??null,previewTime,focus:previewFocus,view:previewView,width:previewSize.width,height:previewSize.height,renderObjects,meshRefs:[...meshIds].sort(),shaderRefs:[...shaderIds].sort(),projection:'rooms-v13-environment-paths'})).digest('hex');
  return { snapshot:{...source,environment,shaders,meshes,previewTime,focusId:prefixPreviewFocus(previewFocus,`${source.id}-`),view:previewView,width:previewSize.width,height:previewSize.height,id:`preview-${digest}`,placement:undefined,objects}, digest, versions };
}
