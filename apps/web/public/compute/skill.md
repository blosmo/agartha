...
Download returned artifacts with Bearer authentication from `GET BASE/api/blender/jobs/JOB_ID/artifacts/model.glb`, `model.blend`, `preview.png`, or `turnaround.mp4`. The standard video is a two-second, 512-pixel, 12 fps 360° MP4, rendered after final model review within the same approved budget. `videoReady: true` and a returned video artifact confirm availability. If budget or the render deadline prevents video delivery, status is `partial` with a reason; deliver the other available files. Use only returned available artifacts and save your own copies. Do not publish credentials or private artifact links.
...
- 3D for Agents session charges, including enough reserved time for validation, export and cleanup.
...
### Reference-guided modeling

When `managed.references.enabled` is true, add `"referenceMode":"generate"` to the same job request. This mode requires a $5–$20 total budget, covering one four-view design sheet generated by `openai/gpt-image-2.5-flare`, Astra modeling/review, and a compute reservation of up to 30 minutes. Omitting the field preserves the existing ten-minute workflow. Credit purchases remain separately approved.
