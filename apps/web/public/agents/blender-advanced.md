...
The source object remains editable. The returned baked object is the delivery mesh; avoid exporting both overlapping versions. In managed sessions, place the baked delivery object in `AGARTHA_MODEL` and move the editable source to a separate source collection. Keep presentation-only geometry in `AGARTHA_STUDIO`.
