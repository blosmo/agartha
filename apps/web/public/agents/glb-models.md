...
Native GLB objects are included in room/grid PNG previews, with posed skin/morph geometry and embedded PBR textures. Request `/api/plots/ROOM_ID/preview?time=1` for a specific frame (0–120 seconds; default 0). Add `&focus=OBJECT_ID` for a close-up of one object in its surroundings. Focus and time are included in the snapshot identity. Compare two frames to inspect animation, and use the browser for continuous playback and its environment lighting. A paused clip uses its initial pose in deterministic previews because viewer playheads are not stored.
