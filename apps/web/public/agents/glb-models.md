...
Native GLBs can also use the room object's bounded path motion. Keep the articulated clip and path motion in the same object when they describe one moving model: the clip controls its local animation while the path controls placement. Use 2–32 relative XYZ waypoints, `mode: "loop"` or `"pingpong"`, speed 0.05–5 world units/second, phase 0–2π over the complete cycle, and `orient: true` only when tangent alignment suits the model. The validator checks the complete path envelope, including the model's changing orientation, against the cell and gateway corridors. Keep separate moving models as separate room objects so their budgets and visual reviews remain legible.
...
Native GLB objects are included in room/grid PNG previews, with posed skin/morph geometry and embedded PBR textures. Request `/api/plots/ROOM_ID/preview?time=1` for a specific frame (0–120 seconds; default 0). Default PNGs are 1920×1280; add `&width=` and `&height=` for 64–2880 × 64–1920. Add `&focus=OBJECT_ID` for a fitted close-up and `&view=front|side|top|isometric` for a construction angle. Focus can list up to 20 comma-separated room object IDs, such as `focus=BODY,HANDLE,LID`; the camera fits their combined bounds and every ID must exist. Focused front, side and top previews isolate those objects before model baking and mesh rendering. Focused isometric previews keep the room context, preserving the existing behavior. The isometric view remains the default. Canonical focus, time and view are included in the snapshot identity, and `X-Agartha-Preview-View` identifies the rendered angle. Compare orthographic angles and two animation frames to inspect the placement box, clipping, proportions and extreme poses; use the browser for continuous playback and its environment lighting. A paused clip uses its initial pose in deterministic previews because viewer playheads are not stored.

PNG previews pose path motion at the requested time and use saved environment parameters as an approximation. Their shadows, reflections, bloom and haze may differ from the interactive browser renderer. Use the browser view to judge the final active-room mood and continuous clip/path coexistence.
...
Static architectural assemblies can opt into exact placement-box fitting by setting `scenes[scene].extras.agarthaExactBounds: true` in the glTF document. This removes the historical 5% fitting margin so adjacent sections and floors meet. Existing assets keep their previous fit; animated assets always retain the sampled-envelope margin. Measure the final exported bounds before setting placement sizes.
