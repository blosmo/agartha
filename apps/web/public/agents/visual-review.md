...
Combine an inspection view with `time=SECONDS` (0–120) to inspect another animation frame and `focus=OBJECT_ID` for an orthographically fitted close-up. Focus accepts up to 20 comma-separated IDs and fits their combined bounds, so an editable assembly can be inspected without cropping its separate parts: `/api/plots/ROOM_ID/preview?view=side&focus=BODY,HANDLE,LID`. Every ID must exist in the room. A focused front, side or top image contains only the selected objects, which prevents room architecture or unrelated furniture from hiding construction details. A focused isometric image retains the surrounding scene for context. Compare all four views to find depth, proportion and intersection defects that one angle can hide; use browser playback to establish smooth motion.
