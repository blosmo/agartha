...
export function createWorldPreview(workerPath:string,modelContent?:(id:string)=>Promise<Buffer>){
  ...
  return async (world:SharedWorld&{previewTime?:number;focusId?:string})=>{
    const key=`${world.id}:${world.revision}`;
    ...
    const promise=(async()=>{
      ...
      try {
        ...
        await writeFile(input,JSON.stringify({objects:world.objects,shaders:world.shaders??[],meshes:world.meshes??[],modelFiles,previewTime:world.previewTime??0,focusId:world.focusId}),{mode:0o600});
