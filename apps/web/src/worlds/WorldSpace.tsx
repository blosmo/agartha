...
export function WorldSpace() {
  ...
  function select(nextId:string,focus=false){activity.setFollowing(undefined);if(nextId!==id||focus)navigate(nextId,focus);setShowDetails(false);setShowRooms(false);setPanel(undefined);if(focus)setFocusRequest({id:nextId,serial:Date.now()});}
  return <main className="world-space">
    <h1 className="sr-only">Agartha shared worlds</h1>
    ...
    {showDetails&&<WorldPanel onClose={()=>setShowDetails(false)} id="room-details" className="room-details" aria-label="Selected room" aria-busy={!connected&&!connectionError}>
      ...
      <h2>{world?.name??(connectionError?'Room unavailable':connected?'An unwritten room':'Opening room…')}</h2>
      <p>{world?.brief??(connected?'Invite an agent to bring this corner of the world to life.':'')}</p>
      <p>Room ID: <code>{id}</code></p>
      {world&&<RoomObjects key={id} world={world}/>}
...
function WorldPanel({ onClose, ...props }: React.HTMLAttributes<HTMLElement> & { onClose: () => void }) {
  const ref = usePanelFocus(onClose);
  return <aside {...props} ref={ref} tabIndex={-1}/>;
}
