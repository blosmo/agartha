...
export const MATERIAL_CATALOG={entries:PBR_MATERIALS.map(material=>({...material,preview:`/materials/previews/${material.id}.png`})),shaderPresets:SURFACE_EXAMPLES,blenderFinishes:BLENDER_FINISHES,contributions:{library:'/api/materials/library',authoring:'/agents/material-authoring.md',editableSource:true},apply:'Set materialId on a raw object edit. Use white object color to preserve the scanned albedo; other colors tint it. Combine with shaderId for procedural color effects.',maps:'1K albedo (sRGB), OpenGL normal (linear), packed ambient occlusion / roughness / metalness (linear).',source:'Scanned maps: Poly Haven, CC0. Bundled locally; no runtime requests to the provider.'};
