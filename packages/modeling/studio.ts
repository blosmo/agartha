...
const actions = ['prepare_generated_asset', 'generate_asset', 'inspect_scene', 'inspect_resources', 'inspect_object', 'edit', 'render_views', 'accept', 'restore', 'finish', 'search_templates', 'inspect_template', 'build_template', 'search_assets', 'load_asset', 'search_polyhaven', 'load_polyhaven', 'prepare_asset', 'publish_asset', 'search_materials', 'load_material', 'prepare_material', 'publish_material'] as const;
...
export function validateStudioImages(value?: StudioImage[], allowSheets = false): StudioImage[] {
  ...
  const allowed = new Set(['reference-front', 'reference-right', 'reference-rear', 'reference-hero', 'asset-reference', ...allowedViews.map(view => `render-${view}`), ...(allowSheets ? ['reference-sheet', 'render-sheet'] : [])]);
...
export function studioRequest(input: { brief: string; history: string; images?: StudioImage[]; remainingCents: number; strategy?: unknown; meshy?: StudioMeshyContext }) {
  ...
  const system = (polyhaven ? SYSTEM : SYSTEM.replace(POLYHAVEN_GUIDANCE, '')) + (input.meshy?.enabled ? '\n' + MESHY_INSTRUCTIONS : '');
  ...
  const enabledActions = actions.filter(action => (polyhaven || action !== 'search_polyhaven' && action !== 'load_polyhaven') && (input.meshy?.enabled || action !== 'prepare_generated_asset' && action !== 'generate_asset'));
  const tool = { ...TOOL, parameters: { ...TOOL.parameters, properties: { ...TOOL.parameters.properties,
    action: { ...TOOL.parameters.properties.action, enum: enabledActions },
    code: { ...TOOL.parameters.properties.code, description: polyhaven ? TOOL.parameters.properties.code.description : TOOL.parameters.properties.code.description.replace('search_polyhaven, load_polyhaven, ', '') },
  } } };
...
export function parseStudioAction(value: unknown): StudioAction {
  ...
  const assetOperation=['prepare_generated_asset','search_templates','inspect_template','build_template','search_assets','load_asset','search_polyhaven','load_polyhaven','prepare_asset'].includes(item.action);
