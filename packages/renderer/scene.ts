...
export function packScene(objects:readonly RenderObject[],seconds=0,definitions:ReadonlyMap<string,RenderMaterial>=new Map()):RenderBatch[] {
  ...
  for(const object of objects){
    ...
    values.push(object.position[0], object.position[1] + pose.lift, object.position[2], pose.yaw,...object.scale,0,...[1,3,5].map(i=>parseInt(object.color.slice(i,i+2),16)/255),1);
...
export function sceneCamera(objects:readonly RenderObject[],aspect:number,minimumRadius=5){
...
export function renderScene(gpu:Gpu,objects:readonly RenderObject[],shader:string,width=960,height=640,shaders:readonly SharedShader[]=[],materials:ReadonlyMap<string,MaterialTextures>=new Map(),pbrShader='',meshes:readonly SharedMesh[]=[],definitions:ReadonlyMap<string,RenderMaterial>=new Map(),seconds=0,focusObjects?:readonly RenderObject[]){
  ...
  const ordered=[...objects].sort((a,b)=>{const ta=Boolean(definitions.get(a.materialId??'')?.transparent),tb=Boolean(definitions.get(b.materialId??'')?.transparent);if(ta!==tb)return Number(ta)-Number(tb);return ta?a.position.reduce((s,n)=>s+n,0)-b.position.reduce((s,n)=>s+n,0):0;});
  ...
  const camera=sceneCamera(focusObjects??objects,width/height,focusObjects?.length ? 0.25 : 5);
  ...
  const draws=batches.map(batch=>{
    ...
    if(definition)source=source.replace('// AGARTHA_PBR',pbrShader).replace('return vec4f(agarthaShade(input.surfacePosition, input.surfaceNormal, input.color, camera.time) * light, 1.0);','return pbrShade(input,front);');
