...
export async function renderWorldPng(objects:readonly RenderObject[],width=960,height=640,shaders:readonly SharedShader[]=[],meshes:readonly SharedMesh[]=[],modelFiles:Readonly<Record<string,string>>={},seconds=0,focusId?:string,view:unknown='isometric'){
  ...
  try {
    ...
    const {output,drawCalls,objectCount}=renderScene(gpu,objects,shader,width,height,shaders,materials,pbrShader,meshes,baked.materials,seconds,focusObjects,previewView);
...
if(input&&output){
  const snapshot=JSON.parse(await readFile(input,'utf8')) as {objects:RenderObject[];shaders?:SharedShader[];meshes?:SharedMesh[];modelFiles?:Record<string,string>;previewTime?:number;focusId?:string;view?:unknown};
  if(!Array.isArray(snapshot.objects))throw new Error('Expected a snapshot with objects.');
  const rendered=await renderWorldPng(snapshot.objects,960,640,snapshot.shaders??[],snapshot.meshes??[],snapshot.modelFiles??{},snapshot.previewTime??0,snapshot.focusId,snapshot.view);
