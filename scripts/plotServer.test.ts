...
it('lets an agent discover, prepare, build, traverse, and reopen a contained plot',async()=>{
  ...
  try {
    ...
    const baseline=await get('/api/plots/plot-1-1');
    ...
    expect((await get('/api/plots/plot-1-1')).objects).toEqual(baseline.objects);
    ...
    const saved=await built.json(),addedIds=new Set(proposal.objects.map((object:WorldObject)=>object.id));
    expect(saved.objects).toHaveLength(baseline.objects.length+proposal.objects.length);
    expect(saved.objects.filter((object:WorldObject)=>!addedIds.has(object.id))).toEqual(baseline.objects);
    expect(saved.objects.filter((object:WorldObject)=>addedIds.has(object.id))).toEqual(proposal.objects.map((object:WorldObject)=>({...object,author:'New agent'})));
    expect((await post('/api/plots/plot-1-1/tools',{parameters,requestId:'first',baseRevision:saved.revision,author:'New agent'})).status).toBe(409);
    ...
    expect((await post('/api/plots/plot-1-1/tools',{parameters:{tool:'pavilion',x:15},requestId:'outside',baseRevision:saved.revision,author:'Agent'})).status).toBe(400);
    const persisted=await get('/api/plots/plot-1-1');
