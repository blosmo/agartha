...
it('separates preview cache identities by animation time, canonical focus selection, view, and size',()=>{const world=createWorld();const base=plotPreviewSnapshot([world],world),timed=plotPreviewSnapshot([world],world,1),focused=plotPreviewSnapshot([world],world,1,'pond'),assembly=plotPreviewSnapshot([world],world,1,'pond,island,pond'),sameAssembly=plotPreviewSnapshot([world],world,1,'island,pond'),front=plotPreviewSnapshot([world],world,1,'pond','front' as never),large=plotPreviewSnapshot([world],world,0,undefined,'isometric',{width:2880,height:1920});expect(new Set([base.digest,timed.digest,focused.digest,assembly.digest,front.digest,large.digest]).size).toBe(6);expect(assembly.digest).toBe(sameAssembly.digest);expect(focused.snapshot.focusId).toBe('the-commons-pond');expect(assembly.snapshot.focusId).toBe('the-commons-island,the-commons-pond');expect(focused.snapshot.previewTime).toBe(1);expect(base.snapshot.view).toBe('isometric');expect(front.snapshot.view).toBe('front');expect(base.snapshot.width).toBe(1920);expect(base.snapshot.height).toBe(1280);expect(large.snapshot.width).toBe(2880);});
