...
export const managedJobTables = {
  ...
  managedInferenceOperations: defineTable({
    jobId: v.string(), operationId: v.string(), executorId: v.string(), payloadFingerprint: v.string(), maxCostCents: v.number(), kind: v.optional(v.union(v.literal("modeling"), v.literal("reference"), v.literal("strategy"), v.literal("review"), v.literal("asset-reference"), v.literal("meshy"))), meshStage: v.optional(v.union(v.literal("image-to-3d"), v.literal("rigging"))), meshParentOperationId: v.optional(v.string()), meshTaskId: v.optional(v.string()), meshResult: v.optional(meshyResult), meshArtifactReady: v.optional(v.boolean()), needsMeshyPoll: v.optional(v.boolean()), meshLastPollAt: v.optional(v.number()), chargeCents: v.optional(v.number()),
