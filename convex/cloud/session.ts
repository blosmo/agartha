...
export async function registerInTransaction(ctx: MutationCtx, args: { token: string; name: string; ipHash: string; recoveryToken?: string }) {
  ...
  if(existing){if(existing.revoked||existing.expiresAt<=Date.now())fail('unauthorized','Agent session expired. Renew or rotate using your recovery credential.');return {agentId:existing.agentId,name:existing.name,expiresAt:existing.expiresAt,recoveryConfigured:Boolean(existing.recoveryTokenHash),governance:await discovery(ctx,'software',existing.agentId)};}
  ...
  return {agentId,name:args.name,expiresAt,recoveryConfigured:Boolean(args.recoveryToken),governance:await discovery(ctx,'software',agentId)};

}
export const register=internalMutation({args:{token:v.string(),name:v.string(),ipHash:v.string(),recoveryToken:v.optional(v.string())},handler:registerInTransaction});
...
export async function maintainInTransaction(ctx: MutationCtx, args: { operation: 'renew' | 'rotate'; token?: string; agentId?: string; recoveryToken?: string; newToken?: string; newRecoveryToken?: string }) {
  ...
  return {agentId:actor.agentId,name:actor.name,expiresAt,credentialVersion:patch.credentialVersion??actor.credentialVersion??0,recoveryConfigured:Boolean(patch.recoveryTokenHash??actor.recoveryTokenHash),governance:await discovery(ctx,'software',actor.agentId)};

}
export const maintain=internalMutation({args:{operation:v.union(v.literal('renew'),v.literal('rotate')),token:v.optional(v.string()),agentId:v.optional(v.string()),recoveryToken:v.optional(v.string()),newToken:v.optional(v.string()),newRecoveryToken:v.optional(v.string())},handler:maintainInTransaction});
