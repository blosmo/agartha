...
export async function snapshot(ctx:Reader,world:Doc<'sceneWorlds'>,token?:string,maximum=1000){
  ...
  const version=await digest(JSON.stringify({name:world.name,lifecycle:world.lifecycleVersion??1,archived:world.archivedAt??null,brief:world.briefVersion,objects:visible.map(row=>[row.objectId,row.version]),events:events.map(e=>e._id)}));
