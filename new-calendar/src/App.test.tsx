...
describe("App", () => {
  it("renders the live timepiece controls and source-framed overlay", () => {
    ...
    expect(within(screen.getByLabelText("Date panel")).queryByText("New Calendar")).not.toBeInTheDocument();
  ...
  it("scrubs time from the New Calendar season bar without duplicating its header text", () => {
    ...
    const comparison = screen.getByLabelText("Date panel");
    const seasonProgress = within(comparison).getByRole("slider", {
  ...
  it("changes the active day when a New Calendar month day is clicked", async () => {
    ...
    const months = screen.getByLabelText("Calendar panel");
    await user.click(within(months).getByRole("button", { name: "Select day 1" }));

    expect(screen.getByDisplayValue("110")).toBeInTheDocument();
    expect(within(months).getByText("Late Spring")).toBeInTheDocument();
  });
